...
const ItemBanner = ({ banner, stt }: ItemBannerProps) => {
  return (
    <tr className="hover:bg-gray-100 dark:hover:bg-gray-700">
      ...
      <td className="text-base font-semibold text-gray-900 dark:text-white">
        <img src={`${SERVER_HOST}/uploads/${banner.image}`} alt="Uploaded" className="w-20 h-20 object-cover rounded" />
      </td>
      <td className="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">{banner.link}</td>
      <td className="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">{banner.position}</td>
      <td className="p-4 text-sm font-normal text-gray-500 whitespace-nowrap dark:text-gray-400">{banner?.category?.name}</td>

      <td className="p-4 space-x-2 whitespace-nowrap">
        <Link
          to={`/admin/crud/edit/banners/${banner.id}`}
