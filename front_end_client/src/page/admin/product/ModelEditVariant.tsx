...
interface ModelEditVariantProps {
...
const ModelEditVariant = ({ setOpenModal, variantId, variants, setLoad }: ModelEditVariantProps) => {
  const {
    register,
    handleSubmit,
    watch,
  ...
  const onSubmit = async (data: VatiantFormValues) => {
    ...
    const variantNews = variants.filter((variant) => variant.id != variantId);
    const isDuplicate = variantNews.some((v) => v.size === variant.size && v.color === variant.color && v.storage === variant.storage);
    if (isDuplicate) {
    ...
    try {
      setIsLoading(true);
      const response = await axios.put(`${SERVER_HOST}/variants/${variantId}`, variant);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const response = await axios.get(`${SERVER_HOST}/variants/${variantId}`);

        const v: IVariant = response.data.data;

        reset({
          price: v.price || 0,
          priceDiscount: v?.priceSale || undefined,
          sale: v.sale ? "true" : "false",
          color: v.color || "",
          size: v.size || "",
          storage: v.storage || "",
          stock: v.stock || 0,
...
export default ModelEditVariant;
