...
const Cart = () => {
  ...
  const handleSelectItem = (variantId: number) => {
    if (selectedItems.includes(variantId)) {
      setSelectedItems(prev => prev.filter(id => id !== variantId));
      setIsSelectAll(false); // bỏ 1 cái thì bỏ luôn check all
    } else {
      const newSelected = [...selectedItems, variantId];
  ...
  const handleUpdateQuantity = (item: ICartItem, quantity: number) => {
    ...
    dispatch(updateQuantity({ variantId: item.variantId, quantity }));
  };
  ...
  const handlePayment = () => {
    if (user) {
      ...
      navigate('/thanh-toan', { state: { selectedCartItems } });
    } else {
    ...
    <div className="min-h-[300px]">
      <div className="custom-container py-3">
        ...
        {cartItems.length == 0 ? (
          <div className="flex flex-col items-center py-6 gap-3">
            <div>
              <svg
                ...
                enableBackground="new 0 0 201.387 201.387"
                xmlSpace="preserve"
              >
                <g>
                  {" "}
          ...
          <div className="col-span-9 grid grid-cols-1 text-sm">
              ...
              <tbody>
                {cartItems.map((item: ICartItem) => (
                  <ItemCart
                    ...
                    onSelect={() => handleSelectItem(item.variantId)}
                    setCart={handleUpdateQuantity}
                  />
          ...
          <div className="col-span-3">
            ...
            {selectedItems.length > 0 && (
              <div className="mt-5 flex">
                <button onClick={handlePayment} className="flex-1 bg-primary text-white py-2 px-4 rounded hover:bg-primary-dark">
                  Thanh toán
                </button>
              </div>
            )}
