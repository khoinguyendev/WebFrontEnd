...
import EditFroalaEditorWord from "../../../components/ui/pagination/EditFroalaEditorWord";
...
const EditBanner = () => {
  ...
  const onSubmit = async (data: BannerFormValues) => {
    ...
    try {
      await axios.put(`${SERVER_HOST}/banners/${id}`, banner);
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const responseBanner = await axios.get(`${SERVER_HOST}/banners/${id}`);
        const bannerData: IBanner = responseBanner.data;
        ...
        reset({
          ...
          position: bannerData.position || "HEADER",
  ...
  return (
    <div className="p-4 bg-white border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
      <h3 className="text-xl font-semibold text-gray-900 dark:text-white">Chỉnh sửa banner</h3>
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-2 gap-6 mt-4">
        ...
        <div className="col-span-2">
          <EditFroalaEditorWord detail={content} setDetail={setContent} />
        </div>
        <div className="col-span-2">
